...
public class AutismServerInfoOverlay extends AutismOverlayBase {
    ...
    public void onCommandSuggestions(int id, net.minecraft.network.protocol.game.ClientboundCommandSuggestionsPacket packet) {
        PluginScanSession capturedSession = activePluginScan;
        PluginProbeSpec probe = capturedSession == null ? null : capturedSession.probes.get(id);
        PluginProbeSpec observed = observedSuggestionRequests.remove(id);
        boolean scanProbe = probe != null || capturedSession != null && capturedSession.pendingProbeIds.contains(id);
        if (!scanProbe && observed == null) return;
        long capturedGeneration = scanProbe && capturedSession != null ? capturedSession.generation : -1L;
        ...
        MC.execute(() -> {
            ...
            if (!liveObservation) {
                String address = currentServerAddress();
                if (session == null || !pluginScanInProgress || !session.matches(capturedGeneration, address)) return;
            }
